...
本專案是一個基於 Python 的工業級 **Siemens S7 通訊協議** 模擬器。利用 `python-snap7` 庫構建，模擬真實 PLC 的記憶體配置（DB, I, Q, M），並內建馬達物理引擎與過載故障 (Overload Trip) 模擬邏輯。

## 1. 系統規範
* **通訊協定**: Siemens S7 Communication (S7Comm)
* **通訊埠 (Port)**: `16003`（若權限不足則自動降級至標準 `102`）
* **位元組順序 (Endianness)**: Big-Endian（符合 Siemens 標準）
...
| 功能 | 位址 | 類型 | 預設值 | 說明 |
| :--- | :--- | :--- | :--- | :--- |
| 馬達啟動控制 | Q0.0 | Bool | True | 寫入 1 啟動、寫入 0 停止 |
| 運轉回授 | I0.0 | Bool | False | 運轉中=1、停止=0 |
| 過載故障 | M10.0 | Bool | False | RPM > 1800 置位 |
...
3. 啟動模擬器：`python main.py`
4. 使用 HMI 或 Client 工具連線至 `127.0.0.1:16003`

### Docker / Compose
* Build/Run:
  * `docker build -t s7-server:1.0 .`
  * `docker run --rm -p 16003:16003 s7-server:1.0`
* Compose:

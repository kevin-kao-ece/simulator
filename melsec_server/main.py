    ...
    def start_simulation_logic(self):
        ...
        def update_loop():
            ...
            while self.running:
                ...
                with self.mem_lock:
                    self.memory[d_code][d15_off : d15_off + 2] = struct.pack('<h', err_scaled)           # D15
                    self.memory[d_code][d16_off : d16_off + 4] = struct.pack('<I', runtime_ticks)       # D16..D17
                    self.memory[d_code][d18_off : d18_off + 4] = struct.pack('<i', encoder_i32)         # D18..D19
                    self.memory[d_code][d20_off : d20_off + 8] = struct.pack('<d', energy_kwh)          # D20..D23
                    self.memory[d_code][d24_off : d24_off + 2] = struct.pack('<H', bus_v_x10)           # D24
                    self.memory[d_code][d25_off : d25_off + 2] = struct.pack('<H', _bcd_pack_u16_u9999(current_rpm))  # D25

                    # Mirrors / other devices for HMI testing
                    self.memory[w_code][200:202] = struct.pack('<H', motor_target_rpm)                   # W100
                    self.memory[tn_code][0:2] = struct.pack('<H', runtime_ticks & 0xFFFF)                # TN0

                    cur_cn = struct.unpack('<H', self.memory[cn_code][0:2])[0]
                    cur_cn = (cur_cn + pulse_inc) & 0xFFFF
                    self.memory[cn_code][0:2] = struct.pack('<H', cur_cn)                                # CN0

                    self.memory[ts_code][0] = 0x01 if (runtime_ticks % 100) >= 50 else 0x00              # TS0

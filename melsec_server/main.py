    ...
    def start_simulation_logic(self):
        ...
        def update_loop():
            ...
            while self.running:
                ...
                self.memory[d_code][20:22] = struct.pack('<H', int(current_rpm))
                ...
                if current_rpm > 0:
                    ...
                    current_amps = 0
                self.memory[d_code][22:24] = struct.pack('<H', int(current_amps))
                ...
                try:
                    ...
                    if cmd == 0x0401:
                        if dev_code in WORD_DEVICE_CODES:
                            ...
                            start, end = head_addr, head_addr + points
                        ...
                        payload = self.memory[dev_code][start:end]
                    ...
                    # --- WRITE LOGIC (1401) ---
                        ...
                        if end <= len(self.memory[dev_code]):
                            ...
                            conn.sendall(self.make_response(b'')) # Success ACK
                            logger.info(f"WRITE {points} pts to {hex(dev_code)} at {head_addr}")
                        else:
                            conn.sendall(self.make_response(b'', error=0xC050))
